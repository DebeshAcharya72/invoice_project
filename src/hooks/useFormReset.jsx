...
const resetForm = ({
  ...
  if (loadParties) {
    loadParties().then(() => {
      // After parties load, set the first party and trigger auto-fill
      setTimeout(() => {
        // Get the updated parties list
        const updatedParties = // You might need to get this from state
          // Set the first party as selected
          setPurchaseForm((prev) => ({
            ...prev,
            party_name: parties[0]?.party_name || "",
          }));

        // Manually trigger destination_from update
        if (parties[0]) {
          const destinationFrom = [
            parties[0].address_line1,
            parties[0].city,
            parties[0].state,
            parties[0].pin,
          ]
          ...
          setVehicleForm((prev) => ({
            ...
            rice_mill_name: parties[0].party_name,
          }));
        }
      }, 100);
    });
